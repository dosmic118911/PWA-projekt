...
<body>
    ...
    <section>
        ...
        $sql = "SELECT id, image, title, category FROM news WHERE archive = 0 and category = 'elections' ORDER BY id DESC LIMIT 5";
    ...
    <section>
        ...
        <div>
        <?php
        $sql = "SELECT id, image, title, category FROM news WHERE archive = 0 and category = 'lesjt' ORDER BY id DESC LIMIT 4";
        $result = $conn->query($sql);
        ...
        if ($result && $result->num_rows > 0) {
            while($row = $result->fetch_assoc()) {
                ...
                echo "<article>
                    <a href ='clanak.php?id=$id'>
                        <img src='slike/$image' alt='JT image'>
                        <h3>$title</h3>
                    </a>
                </article>";               
            }
